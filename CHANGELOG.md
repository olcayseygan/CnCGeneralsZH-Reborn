...
- Hold Ctrl and drag the right button and you draw a line: the selection spreads itself evenly along it, in the order the units already stand, so a column arrives as a firing line instead of a queue. Ticks on the line show where each unit lands before you let go. The camera still scrolls on a plain right drag; if you never scroll that way, `FormationDrag = 2` in Options.ini puts the line on the bare right button, and `0` turns it off.
