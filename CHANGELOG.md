...
- Light vehicles could not hold a straight line: the head of a scout shook the whole way down a road an Overlord drove dead straight. The point a unit steers at sat two cells and a body ahead, taken from whichever sample of the route it happened to be beside, and both halves of that were wrong for a fast chassis. Samples are a cell apart, so the point jumped ten feet forward each time the unit crossed one and the heading jumped with it; it is taken between samples now, off the unit's own unrounded distance along the route, and slides forward a foot at a time. The other half is that thirty feet of warning is forty frames of road for an Overlord and ten for a scout, so the light stuff was steering at something under its nose where a foot of error is a full lock. Moving the point further out for fast units cured the shake and cost four times the units left stuck - a point fifty feet up the road is measured against the ground fifty feet up the road, and nobody asks about the narrow bit in between - so the chassis difference is dealt with in the wheel instead. The rate a unit turns towards its heading is now cut by how much of that thirty feet it eats in half a second: unchanged for anything heavy, about halved for a scout. It is a lag rather than a different place to drive at, which is the part the other version got wrong.
- Vehicles walked off the side of bridges and queued up beside the entrance to them instead of driving on. A unit driving in a crowd holds a line a set distance off the middle of the road, and the road's width is measured by asking the ground either side whether a tank fits on it. On a bridge that question was being asked of the riverbank underneath. So the road read as sixty feet wide over a deck barely wide enough for two tanks, and a line held a body width off centre walked a column into the water - and on the approach, the same line put the leading tank against the abutment rather than the entrance, with everybody behind it stopped on the bank. The width is now measured on the deck the route actually runs over, and both the bridge and forty feet of road each side of it have no width at all: every unit drives the middle of the road across a bridge, which is what the crossing is for.
- Infantry can walk through infantry again. Two soldiers crossing have never blocked each other in this game - only two going the same way do - and the crowd rules had quietly taken that back: a squad crossing another squad read as a wall, and every man in it braked, gave way and shuffled sideways for a crowd he was supposed to walk straight into. The crowd now uses the game's own rule for that, so a column following a column is still traffic and two squads crossing pass through one another.
