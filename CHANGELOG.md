...
- Drag the right button and you draw a line: the selection spreads itself evenly along it, in the order the units already stand, so a column arrives as a firing line instead of a queue. It is the line you actually drew, curves and all, so you can bend a defence round a hillside or pour men along a wall instead of only across a straight run. Ticks on it show where each unit lands before you let go, and letting go over the sidebar finishes the order rather than leaving the line stuck to the cursor. This is what the right button freed up. `FormationDrag = no` in Options.ini turns it off if you would rather a slipped click did nothing.
