...
public:
	...
	// Plain memory standing in for the D3D buffer when there is no render device.  Everything that
	// fills a buffer does so through the lock classes, and a run with no picture still fills them -
	// the terrain, the trees and the bridges all build theirs while the map loads, long before
	// anything would be drawn.  Writing into this instead keeps every one of those callers honest
